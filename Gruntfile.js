...
module.exports = function (grunt) {
    ...
    grunt.registerTask('enforce', `Install
    commit
    message
    enforce
    script
...
function findModule(name) {
    ...
    var module = {
        name: name,
        moduleName: enquote(`ui.bootstrap.$
    {
        name
    }`),
...
:
    ...
    function processCSS(moduleName, state, minify, file) {
        ...
        css = css
            ...
            .replace(/'/g, "\\'")
            .replace(/\r?\n/g, '\\n');
        js = `angular.module('ui.bootstrap.${moduleName}').run(function () {
            !angular.$$csp().noInlineStyle && angular.element(document).find('head').prepend('<style type="text/css">${css}</style>');
        });`;
        state.js.push(js);
